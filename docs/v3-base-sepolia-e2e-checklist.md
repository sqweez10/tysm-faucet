 ...
 * [ ] Contract Deployment
   ...
   * [ ] Deploy TYSMFaucetV3 to Base Sepolia referencing MockTYSM, SIGNER_ADDRESS, and OWNER_ADDRESS.
 ...
 * [ ] Negative & Edge Case Verification
   ...
   * [ ] Reused signature/nonce on-chain ("Authorization already used").
   * [ ] Wallet / FID mismatch.
   * [ ] Missing marker in Farcaster cast.
   * [ ] Expired deadline ("Signature expired").
   * [ ] Blocklisted wallet/FID.
   * [ ] Paused contract ("Faucet is paused").
...
TYSM_V3_SEPOLIA_CONTRACT_ADDRESS=<BASE_SEPOLIA_V3_CONTRACT_ADDRESS>
...
UPSTASH_REDIS_REST_URL=<UPSTASH_TEST_REDIS_REST_URL>
UPSTASH_REDIS_REST_TOKEN=<UPSTASH_TEST_REDIS_REST_TOKEN>

3. Deployment Steps on Base Sepolia
> Note: Adjust contract paths (e.g., src/... or contracts/...) to match your actual deployment project layout.
> 
...
Deploy the V3 contract passing the three required constructor arguments (_tysm, _signer, _owner):
forge create --rpc-url https://sepolia.base.org \
  --private-key <DEPLOYER_PRIVATE_KEY> \
  src/TYSMFaucetV3.sol:TYSMFaucetV3 \
  --constructor-args <MOCK_TYSM_ADDRESS> <TYSM_V3_SEPOLIA_SIGNER_ADDRESS> <OWNER_ADDRESS>
...
| 9.2 | Reused Signature/Nonce | Call claimWithSignature twice on-chain using the same parameters. | On-chain revert: "Authorization already used" |
...
| 9.6 | Expired Deadline | Call claimWithSignature after deadline timestamp has passed. | On-chain revert: "Signature expired" |
| 9.7 | Blocklisted Wallet | Add wallet to Upstash tysm:v3:deny:wallet:<address> and issue request. | HTTP 403: not_eligible |
| 9.8 | Paused Faucet | Call pause() on contract via owner key, then execute claimWithSignature. | On-chain revert: "Faucet is paused" |
...
B. On-Chain Revert: "Invalid signer"
 ...
 * Solution: Query contract.signer() on-chain to verify it matches the API signer.

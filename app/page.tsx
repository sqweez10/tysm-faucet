...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setCountdown((p) => { if (p <= 1) { refetchCanClaim(); refetchTimeLeft(); return 0; } return p - 1; });
    }, 1000);
      ...
      <div className="max-w-sm mx-auto px-4 pt-5 pb-8 space-y-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            {userCtx?.user?.pfpUrl ? (
              <img src={userCtx.user.pfpUrl} alt="avatar" className="w-10 h-10 rounded-full border-2 border-yellow-500/40 object-cover" />
            ) : (
          ...
          <div className="bg-white/5 border border-yellow-800/30 rounded-xl px-3 py-1.5 text-right">
            <p className="text-gray-500 text-[9px] uppercase tracking-widest">Streak</p>
            <p className="text-yellow-400 font-black text-lg leading-tight">{streak}d <Flames streak={streak} /></p>
          </div>
        </div>

        <div className="text-center py-2">
          <div className="text-5xl mb-1.5" style={{ animation: "float 3s ease-in-out infinite" }}>🙏</div>
        ...
        <div className="bg-white/4 border border-yellow-900/20 rounded-2xl p-3.5">
          <div className="flex justify-between items-center mb-2">
            ...
            <p className="text-yellow-400 text-xs font-black">{streak}/7 {streak >= 7 ? "🔥 BONUS!" : "days"}</p>
          </div>
          <div className="w-full bg-gray-800/80 rounded-full h-2.5 overflow-hidden">
            <div className="h-2.5 rounded-full transition-all duration-700" style={{ width: `${streakPct}%`, background: streak >= 7 ? "linear-gradient(90deg,#ef4444,#f59e0b)" : "linear-gradient(90deg,#78350f,#f59e0b)" }} />
          </div>
          <p className="text-gray-600 text-[10px] mt-1.5">{streak >= 7 ? "🎉 +500 bonus every day!" : `${7 - streak} more days → +500 $TYSM bonus`}</p>
        </div>

        <div className="bg-white/4 border border-yellow-900/20 rounded-2xl p-4 text-center" style={{ boxShadow: "0 0 28px rgba(245,158,11,0.12)" }}>
          {!contractReady ? (
            ...
            <>
              ...
              {streak >= 7 && (
                <div className="bg-orange-950/40 border border-orange-700/30 rounded-xl py-1.5 px-3 mb-3">
                  <p className="text-orange-400 text-[11px] font-bold">🔥 +500 streak bonus included!</p>
                </div>
              )}
            ...
            <>
              ...
              <p className="font-black text-yellow-400 font-mono" style={{ fontSize: 40, letterSpacing: "0.06em" }}>{countdown > 0 ? formatCountdown(countdown) : "00:00:00"}</p>
        ...
        <div className="bg-white/3 border border-white/5 rounded-2xl p-3.5 space-y-1.5">
          ...
          {["🙏 Claim 2,000 $TYSM every 24 hours", "🔥 7-day streak → +500 $TYSM bonus daily", "⚡ Share to grow the TYSM community", "🔒 Smart Contract on Base Chain"]
            .map((t, i) => <p key={i} className="text-gray-600 text-[11px]">{t}</p>)}
        </div>

...
// Milestone config
const MILESTONES = [
  { day: 7,  reward: "10,000",  emoji: "🎁",    color: "#f59e0b" },
  { day: 15, reward: "40,000",  emoji: "🎁🎁",  color: "#10b981" },
  { day: 30, reward: "90,000",  emoji: "🎁🎁🎁",color: "#8b5cf6" },
];

function getNextMilestone(streak: number) {
  if (streak < 7)  return { day: 7,  reward: "10,000",  emoji: "🎁",    daysLeft: 7 - streak };
  if (streak < 15) return { day: 15, reward: "40,000",  emoji: "🎁🎁",  daysLeft: 15 - streak };
  if (streak < 30) return { day: 30, reward: "90,000",  emoji: "🎁🎁🎁",daysLeft: 30 - streak };
  return null;
}
...
export default function Home() {
  ...
  const handleShareFirst = useCallback(async () => {
    ...
    const text = nextM
      ? `🙏 About to claim ${reward} $TYSM on Day ${streak + 1}!\n🎯 Only ${nextM.daysLeft} days left → ${nextM.emoji} ${nextM.reward} $TYSM Milestone!\n\nCome claim free every day:`
      : `🙏 ${name} is about to claim $TYSM!\nStreak Day ${streak + 1} 🔥\n\nCome claim free every day:`;
  ...
  const handleShareAfter = useCallback(async () => {
    ...
    const text = isMile
      ? `🎉 ${name} reached Milestone Day ${streak}!\nReceived ${reward} $TYSM today! 🎁\nTotal claimed: ${total} $TYSM\n\nCome claim free every day:`
      : `🙏 ${name} claimed ${reward} $TYSM!\nDay ${streak} streak 🔥\nTotal claimed: ${total} $TYSM\n\nCome claim free every day:`;
  ...
  // Progress bar — position within 30-day cycle
      ...
      <div className="max-w-sm mx-auto px-4 pt-5 pb-8 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-white/4 border border-yellow-900/20 rounded-2xl p-3">
            ...
            <p className="text-yellow-400 font-black text-base leading-tight">{contractReady ? formatAmount(faucetBal) : "—"}</p>
          ...
          {nextM && (
            <div className="mt-2 bg-white/5 rounded-xl px-3 py-1.5 flex justify-between items-center">
              ...
              <p className="text-yellow-400 text-[10px] font-bold">
                {nextM.emoji} Day {nextM.day} → {nextM.reward} $TYSM ({nextM.daysLeft}d left)
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white/4 border border-yellow-900/20 rounded-2xl p-4 text-center"
          ...
          {!contractReady ? (
            ...
            <>
              ...
              {isTxSuccess && justClaimed && <p className="text-green-400 text-xs mb-2 font-semibold">✅ Claimed! Check your wallet.</p>}
              ...
              {!hasShared ? (
                <>
                  ...
                  <p className="text-gray-600 text-[10px]">Please share first to unlock your claim 🙏</p>
                </>
              ) : (
            ...
            <>
              ...
              {nextM && (
                <p className="text-yellow-700 text-[10px] mt-1">
                  🎯 {nextM.daysLeft} days left → {nextM.emoji} {nextM.reward} $TYSM!
                </p>
              )}
            </>
          )}
        ...
        <div className="bg-white/4 border border-purple-900/20 rounded-2xl p-3">
          <div className="flex justify-between items-center">
            ...
            <div className="text-right">
              ...
              <p className="text-yellow-400 font-black text-xl">{contractReady ? formatAmount(faucetBal) : "—"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/3 border border-white/5 rounded-2xl p-3.5">
          <p className="text-gray-400 text-xs font-bold mb-2">🎁 Milestone Rewards</p>
          {[
            { day: 7,  reward: "10,000", emoji: "🎁",    color: "text-yellow-400" },
            { day: 15, reward: "40,000", emoji: "🎁🎁",  color: "text-green-400"  },
            { day: 30, reward: "90,000", emoji: "🎁🎁🎁",color: "text-purple-400" },
          ].map((m) => (
            <div key={m.day} className="flex justify-between items-center py-0.5">
              <p className="text-gray-600 text-[11px]">{m.emoji} Day {m.day}</p>
              <p className={`text-[11px] font-bold ${m.color}`}>{m.reward} $TYSM</p>
            </div>
          ))}
          <div className="mt-2 border-t border-white/5 pt-2">
            <p className="text-gray-600 text-[10px]">🔄 After Day 30 → Cycle resets</p>
            <p className="text-gray-600 text-[10px]">⚡ Must share before claiming</p>
          </div>
        </div>

        <p className="text-center text-gray-700 text-[10px] pt-1">$TYSM Faucet · tops87 · Base Chain</p>
      </div>
    </main>
  );
      }

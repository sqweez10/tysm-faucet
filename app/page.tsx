...
export default function Home() {
  ...
  const handleShareFirst = useCallback(async () => {
    ...
    const text = nextM
      ? `🙏 กำลังจะ Claim ${reward} $TYSM วันที่ ${streak + 1}!\n🎯 อีก ${nextM.daysLeft} วัน → ${nextM.emoji} ${nextM.reward} $TYSM Milestone!\n\nมา Claim ฟรีทุกวันได้เลย:`
      : `🙏 ${name} กำลังจะ Claim $TYSM!\nStreak วันที่ ${streak + 1} 🔥\n\nมา Claim ฟรีทุกวันได้เลย:`;
  ...
  const handleShareAfter = useCallback(async () => {
    ...
    const text = isMile
      ? `🎉 ${name} ถึง Milestone Day ${streak}!\nได้รับ ${reward} $TYSM วันนี้! 🎁\nTotal: ${total} $TYSM\n\nมา Claim ฟรีทุกวัน:`
          ...
          {nextM && (
            <div className="mt-2 bg-white/5 rounded-xl px-3 py-1.5 flex justify-between items-center">
              ...
              <p className="text-yellow-400 text-[10px] font-bold">
                {nextM.emoji} Day {nextM.day} → {nextM.reward} $TYSM ({nextM.daysLeft} วันอีก)
              </p>
            </div>
          )}
        </div>
              ...
              {!hasShared ? (
                <>
                  <button onClick={handleShareFirst}
                    ...
                    ⚡ แชร์ก่อน → ปลดล็อค Claim!
                  </button>
                  <p className="text-gray-600 text-[10px]">แชร์ก่อนถึงจะ Claim ได้ครับ 🙏</p>
                </>
              ) : (
            ...
            <>
              ...
              {nextM && (
                <p className="text-yellow-700 text-[10px] mt-1">
                  🎯 อีก {nextM.daysLeft} วัน → {nextM.emoji} {nextM.reward} $TYSM!
                </p>
              )}
            </>
          )}
        ...
        <div className="bg-white/3 border border-white/5 rounded-2xl p-3.5">
          ...
          {[
            ...
            <div key={m.day} className="flex justify-between items-center py-0.5">
              <p className="text-gray-600 text-[11px]">{m.emoji} วันที่ {m.day}</p>
              <p className={`text-[11px] font-bold ${m.color}`}>{m.reward} $TYSM</p>
            </div>
          ))}
          <div className="mt-2 border-t border-white/5 pt-2">
            <p className="text-gray-600 text-[10px]">🔄 หลังวันที่ 30 → เริ่ม Cycle ใหม่</p>
            <p className="text-gray-600 text-[10px]">⚡ แชร์ก่อนถึงจะ Claim ได้</p>
          </div>
        </div>
